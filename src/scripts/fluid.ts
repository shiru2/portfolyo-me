...
// bloom glow that the cursor splats and stirs.
// Algorithm (splat → curl → vorticity → divergence → pressure → gradient-subtract
// → advect) is the standard stable-fluids pipeline; shaders here are our own.
// WebGL2 + half-float FBO ping-pong. Deferred + capability-gated; static CSS
// poster is the fallback.
import { Renderer, RenderTarget, Program, Mesh, Triangle } from 'ogl';
import { state as fluidState } from './fluid-state';
...
uniform vec3 uColor;
void main(){
  float big = fbm(vUv * 1.7 + vec2(uTime * 0.026, -uTime * 0.019));   // broad cloud cover
  float fine = fbm(vUv * 4.2 + vec2(-uTime * 0.034, uTime * 0.038));   // softer secondary structure
  float n = smoothstep(0.32, 0.91, big) * (0.55 + 0.45 * fine);
...
function startSim(canvas: HTMLCanvasElement): void {
  ...
  function resize() {
    ...
    if (portrait) {
      ...
      bloomBase = 0.62;                   // less glow spreading blue across the narrow frame
      blackBase = 0.05;                   // crush faint haze so the dark base reads
      srcSize = 0.58;
      dyeMul = 0.52;
    } else {
    ...
    display.u.uBloomAmt.value = bloomBase * fluidState.bloomAmt;
    display.u.uBlack.value = blackBase + fluidState.blackPoint;
  }

  // ---- colour: cool-leaning, dim → smoky, not neon ----
  function lapis(j: number): [number, number, number] { return [fluidState.colorR * j, fluidState.colorG * j, fluidState.colorB * j]; }
  ...
  // ---- wind: 6 patterns (1 source ×4, 2 sources ×2), each at random
  // positions, smoothly morphing to a new random pattern on the user-tuned cycle ----
  type WindPattern = { p: [number, number][]; w: number[] };
  const WIND_SPEEDS = [1.0, 0.74, 0.50];               // each source picks one: current speed is the max
  ...
  for (let i = 0; i < 4; i++) WIND.push(buildWind(1));
  for (let i = 0; i < 2; i++) WIND.push(buildWind(2));
  ...
  function step(now: number) {
    ...
    const T = (now - t0) / 1000;
    const ambientCount = Math.max(1, Math.min(NE, Math.round(fluidState.ambientCount)));
    const ambientSize = srcSize * fluidState.ambientSize;
    ...
    if (T - windT0 >= windHold) {
      windT0 += windHold;
      windPrev = windCur;
      let n = windCur; while (n === windCur) n = (Math.random() * WIND.length) | 0;
      windCur = n;
    }
    const k = Math.min(1, (T - windT0) / windMorph);
    ...
    if (!cursorSuppressed && (now - input.lastMove) < 650) {
      ...
      doSplat(emitX, emitY, (emitX - prevEmitX) * fluidState.cursorForce, (emitY - prevEmitY) * fluidState.cursorForce, lapis((0.05 + 0.09 * srcSize) * fr), 0.011 * fluidState.cursorSize);
      cursorActive = true;
    ...
    vortP.u.uVelocity.value = velocity.read.texture; vortP.u.uCurl.value = curl.texture; vortP.u.uDt.value = dt; vortP.u.uCurlAmt.value = fluidState.vorticity; pass(vortP, velocity.write); velocity.swap();
    ...
    advect.u.uVelocity.value = velocity.read.texture; advect.u.uSource.value = dye.read.texture; advect.u.uDissipation.value = fluidState.dyeDissipation;
    pass(advect, dye.write); dye.swap();

    // ---- bloom (prefilter → separable blur) ----
    prefilter.u.uThreshold.value = fluidState.bloomThreshold;
    prefilter.u.uKnee.value = fluidState.bloomKnee;
    ...
    // ---- display ----
    display.u.uDye.value = dye.read.texture; display.u.uBloom.value = bloom.read.texture; display.u.uBloomAmt.value = bloomBase * fluidState.bloomAmt; display.u.uBlack.value = blackBase + fluidState.blackPoint;
